<template>
  <div>
    <b-container class="mb-2">
      <b-card class="shadow-sm">
        <b-card-body>
          <b-row>
            <b-col lg="6">
              <h2 class="text-primary font-weight-bold">Pupils (Trash)</h2>
            </b-col>
    ...
    >
      ...
      >Are you sure you want to permanently delete these pupils?</p>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  computed: {},
  mounted() {
    this.getDeletedPupils();
  },
  methods: {
    getDeletedPupils: function(page) {
      const deletedPupilsAPI = `${this.host}/pupils/deleted?search=${this.search}&limit=${this.limit}&page=${page}`;
      axios
        .get(deletedPupilsAPI)
        .then(response => {
    ...
    restoreConfirm: function() {
      let id = this.pupils[this.restore_index].id;
      const deletedPupilsAPI = `${this.host}/pupils/restore`;
      const data = {
        key: id
      };
      axios
        .post(deletedPupilsAPI, data)
        .then(response => {
          if (response.data.status == 201) {
            this.pupils.splice(this.restore_index, 1);
    ...
    removeConfirm: function() {
      let id = this.pupils[this.delete_index].id;
      const deletedPupilsAPI = `${this.host}/pupils/delete/${id}`;
      axios
        .delete(deletedPupilsAPI)
        .then(response => {
          if (response.data.status == 201) {
